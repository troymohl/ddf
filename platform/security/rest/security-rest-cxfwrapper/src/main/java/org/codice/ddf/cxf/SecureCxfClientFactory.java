...
public class SecureCxfClientFactory<T> {
    ...
    if (keyInfo != null) {
      ...
      try {
        tlsParams.setSSLSocketFactory(
            getSSLSocketFactory(keyInfo.getAlias(), keyManagers, trustManagers));
      } catch (KeyManagementException | NoSuchAlgorithmException e) {
        LOGGER.debug("Unable to override default SSL Socket Factory");
      }
    } else {
  ...
  private SSLSocketFactory getSSLSocketFactory(
      String alias, KeyManager[] keyManagers, TrustManager[] trustManagers)
